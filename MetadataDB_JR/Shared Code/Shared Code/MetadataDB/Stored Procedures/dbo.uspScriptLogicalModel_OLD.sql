...
-- ====================================================================================
CREATE PROCEDURE [dbo].[uspScriptLogicalModel_OLD]
(
...
SET ANSI_WARNINGS OFF
	...
	BEGIN TRY
		...
		SELECT
			...
			--(SELECT COUNT(*) FROM dbo.DWElement DWElement WHERE DWElement.DWObjectID = DWObject.DWObjectID AND DWElement.EntityLookupObjectID IS NOT NULL) AS RelationshipCount
		FROM
		...
		WHILE (SELECT COUNT(*) FROM #DWSchemaList_temp) > 0
			BEGIN
				...
				SELECT @Sql_DW_Schemas = @Sql_DW_Schemas + 'IF  NOT EXISTS (SELECT * FROM sys.schemas WHERE name = N''' + @DWLayerAbbreviation + ''')
